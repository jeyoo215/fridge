...
public class ShoppingListController {
    ...
    public void addMissingIngredients(@RequestParam Long userId, @RequestParam Long recipeId) {
        shoppingListService.addMissingIngredientsToMyList(userId, recipeId);
    }

    // 내 장보기 리스트 전체 조회
    @GetMapping("/me")
    public MyShoppingListResponse getMyShoppingList(@RequestParam Long userId) {
        return shoppingListService.getMyShoppingList(userId);
    }

    @PatchMapping("/items/{itemId}/check")
    public void checkItem(@RequestParam Long userId, @PathVariable Long itemId) {
        shoppingListService.checkItem(userId, itemId);
    }

    @PatchMapping("/items/{itemId}/uncheck")
    public void uncheckItem(@RequestParam Long userId, @PathVariable Long itemId) {
        shoppingListService.uncheckItem(userId, itemId);
    }

    @DeleteMapping("/items/{itemId}")
    public void deleteItem(@RequestParam Long userId, @PathVariable Long itemId) {
