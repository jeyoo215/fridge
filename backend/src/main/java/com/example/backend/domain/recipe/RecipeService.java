        ...
        public Long createRecipe(RecipeCreateRequest request) {
                RecipeCategory category = recipeCategoryRepository.findById(request.categoryId())
                                .orElseThrow(() -> new EntityNotFoundException(
                                                "존재하지 않는 카테고리입니다. id=" + request.categoryId()));
                ...
                for (RecipeCreateRequest.IngredientItem item : request.ingredients()) {
                        Ingredient ingredient = ingredientRepository.findById(item.ingredientId())
                                        .orElseThrow(() -> new EntityNotFoundException(
                                                        "존재하지 않는 재료입니다. id=" + item.ingredientId()));
        ...
        public List<RecipeRecommendResponse> recommendRecipes(Long userId) {
                ...
                Map<Long, Long> dDayByIngredientId = myIngredients.stream()
                                .collect(Collectors.toMap(
                                                ...
                                                userIngredient -> ChronoUnit.DAYS.between(LocalDate.now(),
                                                                userIngredient.getExpirationDate()),
                                                (existing, duplicate) -> existing // 혹시 같은 재료 여러 개 보유 시 첫 값 사용
                                ));

                List<RecipeRepository.RecipeMatchResult> matchResults = recipeRepository
                                .findRecipesByMatchingIngredients(ingredientIds);
                ...
                return recipeRepository.findAllById(recipeIds).stream()
                                .map(recipe -> {
                                        ...
                                        int expiryPriorityScore = calculateExpiryPriorityScore(recipe,
                                                        dDayByIngredientId);
                                ...
                                .sorted(
                                                java.util.Comparator
                                                                .comparingInt(RecipeRecommendResponse::getExpiryPriorityScore)
                                                                .reversed()
                                                                .thenComparing(java.util.Comparator.comparingLong(
                                                                                RecipeRecommendResponse::getMatchCount)
                                                                                .reversed()))
                                .toList();
        }
        ...
        private int calculateExpiryPriorityScore(Recipe recipe, Map<Long, Long> dDayByIngredientId) {
                return recipe.getRecipeIngredients().stream()
                                .mapToInt(recipeIngredient -> {
                                        Long dDay = dDayByIngredientId
                                                        .get(recipeIngredient.getIngredient().getIngredientId());
                                        if (dDay == null)
                                                return 0; // 보유하지 않은 재료면 가중치 없음
                                        if (dDay <= 1)
                                                return EXPIRY_WEIGHT_D1;
                                        if (dDay <= 3)
                                                return EXPIRY_WEIGHT_D3;
                                        return 0;
                                })
                                .sum();
        }
