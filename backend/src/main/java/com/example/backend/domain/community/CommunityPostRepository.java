...
public interface CommunityPostRepository extends JpaRepository<CommunityPost, Long> {
    ...
    // 신고 누적으로 hidden=true가 된 글은 목록에서 제외한다 (관리자 검토 전까지).
    @Query("SELECT p.postId FROM CommunityPost p WHERE p.boardType = :boardType AND p.hidden = false ORDER BY p.createdAt DESC")
    ...
    @Query("SELECT p.postId FROM CommunityPost p WHERE p.boardType = :boardType AND p.hidden = false ORDER BY p.likeCount DESC, p.createdAt DESC")
    ...
    @Query("SELECT p.postId FROM CommunityPost p WHERE p.boardType = :boardType AND p.prefix = :prefix AND p.hidden = false ORDER BY p.createdAt DESC")
    ...
    @Query("SELECT p.postId FROM CommunityPost p WHERE p.boardType = :boardType AND p.prefix = :prefix AND p.hidden = false ORDER BY p.likeCount DESC, p.createdAt DESC")
    ...
    @Query("SELECT p.postId FROM CommunityPost p WHERE p.boardType = :boardType AND REPLACE(p.title, ' ', '') LIKE CONCAT('%', :keyword, '%') AND p.hidden = false ORDER BY p.createdAt DESC")
    ...
    @Query("SELECT p.postId FROM CommunityPost p WHERE p.boardType = :boardType AND REPLACE(p.title, ' ', '') LIKE CONCAT('%', :keyword, '%') AND p.hidden = false ORDER BY p.likeCount DESC, p.createdAt DESC")
