-- =========================================================
-- 냉장고 파먹기 (Fridge to Table) - ERD 참고용 스키마
-- 주의: 이 파일은 문서/설계 참고용입니다. 실제 DB에는 실행하지 않습니다.
-- 테이블은 JPA 엔티티(@Entity)가 ddl-auto=update로 생성합니다.
-- (이전 backend/src/main/resources/schema.sql 에 있던 내용을 여기로 이동)
-- =========================================================
...
CREATE TABLE user_ingredient (
    ...
    status              VARCHAR(20) NOT NULL DEFAULT '보유중',  -- 실제 컬럼명은 엔티티 기준(status, Enum: 보유중/소진/폐기)
    created_at          DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at          DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE recipe (
    ...
    difficulty            VARCHAR(20) NULL,             -- 쉬움, 보통, 어려움
    servings              INT NULL,
